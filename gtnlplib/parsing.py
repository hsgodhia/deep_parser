...
class ParserState:
    ...
    def done_parsing(self):
        ...
        """
        # STUDENT
        if self.input_buffer[self.curr_input_buff_idx].headword == END_OF_INPUT_TOK and len(self.stack) == 1:
            return True
        else:
            return False
        # END STUDENT
    ...
    def _reduce(self, action):
        ...
        # hint: use list.pop()
        # END STUDENT
        top_wrd = self.stack.pop()
        sec_top_wrd = self.stack.pop()

        if action == Actions.REDUCE_L:
            #its a left reduce action
            head = top_wrd
            modifier = sec_top_wrd
        elif action == Actions.REDUCE_R:
            #its a right reduce action
            head = sec_top_wrd
            modifier = top_wrd

        self.stack.append(StackEntry(head.headword, head.headword_pos, self.combiner(head.embedding, modifier.embedding)))
        depgraph = DepGraphEdge((head.headword, head.headword_pos), (modifier.headword, modifier.headword_pos))		
        return depgraph
...
class TransitionParser(nn.Module):
    ...
    def forward(self, sentence, actions=None):
        ...
        while parser_state.done_parsing() is False:
            fts = self.feature_extractor.get_features(parser_state)
            probs = self.action_chooser(fts)
            outputs.append(probs)
            if have_gold_actions:
                act = action_queue.popleft()                
            else:
                act = utils.argmax(probs)
                if parser_state.input_buffer_len() < 2:
                    if act == Actions.SHIFT:
                        act = Actions.REDUCE_R
                if parser_state.stack_len() < 2:
                    if act == Actions.REDUCE_R or act == Actions.REDUCE_L:
                        act = Actions.SHIFT
            actions_done.append(act)
            if act == Actions.SHIFT:
                parser_state.shift()
            elif act == Actions.REDUCE_L:
                arc = parser_state.reduce_left()
                dep_graph.add(arc)
            elif act == Actions.REDUCE_R:
                arc = parser_state.reduce_right()
                dep_graph.add(arc)
        # END STUDENT
